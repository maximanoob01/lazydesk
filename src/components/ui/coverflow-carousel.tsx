// "use client" is a Next.js directive — safe to leave as a comment in Vite
// "use client";
...
export interface CoverflowCarouselProps {
  ...
  /** Auto-advance every N ms. Set to 0 to disable. */
  autoPlay?: number;
...
export function CoverflowCarousel({
  ...
  // Auto-advance: tick every `autoPlay` ms, but pause while the user
  // is hovering over the frame or actively dragging.
  React.useEffect(() => {
    if (!autoPlay) return;
    const id = setInterval(() => {
      if (!isPaused.current) nudge(1);
    }, autoPlay);
  ...
  return (
    ...
    >
      ...
      {showCaption && active?.title && (
        ...
        >
          <p className="text-[15px] font-semibold tracking-tight text-current">
            {active.title}
          </p>
          {active.subtitle && (
            <p className="mt-1 text-[13px] text-current/70">
              {active.subtitle}
            </p>
          )}
